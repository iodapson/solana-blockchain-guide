...
- Update your `new PublicKey()` string value inside your `votingdapp.spec.ts` with the generated `Program Id` shown inside your terminal.

- Set up your poll and candidate ID configuration inside your `votingdapp.spec.ts` in order to create votes. Make the following updates to your `votingdapp.spec.ts` code:
...
- Run `$ anchor clean` inside your terminal to cleanup your workspace and ensure that everything is set up for success.

- Then finally rerun `$ anchor test --skip-local-validator`
